...
class PlayState extends MusicBeatState
{
	...
	override public function create()
	{
		...
		switch (SONG.song.toLowerCase())
		{
			...
			case 'pato gallina':
				dialogue = CoolUtil.coolTextFile(Paths.txt('pato gallina/pato gallinaDialogue'));
			case 'foxy':
				dialogue = CoolUtil.coolTextFile(Paths.txt('foxy/foxyDialogue'));
			case 'hombre morado':
				dialogue = CoolUtil.coolTextFile(Paths.txt('hombre morado/hombre moradoDialogue'));
		}
		...
		switch(SONG.stage)
		{
			...
			case 'pradera':
			{
					...
					var bg:FlxSprite = new FlxSprite(-150).loadGraphic(Paths.image('pradera/sky'));
					bg.scrollFactor.set(0.1, 0.1);
					add(bg);

					var city:FlxSprite = new FlxSprite(-35).loadGraphic(Paths.image('pradera/city'));
					city.scrollFactor.set(0.3, 0.3);
					city.setGraphicSize(Std.int(city.width * 0.85));
					city.updateHitbox();
					add(city);

					var streetBehind:FlxSprite = new FlxSprite(-40, 50).loadGraphic(Paths.image('pradera/behindTrain'));
					add(streetBehind);

					var street:FlxSprite = new FlxSprite(-50, streetBehind.y).loadGraphic(Paths.image('pradera/street'));
					add(street);
